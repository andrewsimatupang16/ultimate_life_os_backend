...
class MicroBreakTaskCreate(BaseModel):
    title: str = Field(..., min_length=1, max_length=180)
    hint: Optional[str] = Field(None, max_length=240)
    duration_key: str = Field(..., min_length=1, max_length=32)

    @field_validator("title", "duration_key", mode="before")
    @classmethod
    def normalize_required_text(cls, value):
        return strip_non_empty_string(value, "micro break task field")
...
class MicroBreakTaskUpdate(BaseModel):
    title: Optional[str] = Field(None, min_length=1, max_length=180)
    hint: Optional[str] = Field(None, max_length=240)
    duration_key: Optional[str] = Field(None, min_length=1, max_length=32)

    @field_validator("title", "duration_key", mode="before")
    @classmethod
    def normalize_optional_required_text(cls, value):
        if value is None:
            return None
        return strip_non_empty_string(value, "micro break task field")
...
class MicroBreakTaskResponse(BaseModel):
    id: UUID
    title: str
...
class MicroBreakCompleteRequest(BaseModel):
    task_id: str = Field(..., min_length=1, max_length=96)
    task_title: str = Field(..., min_length=1, max_length=180)
    duration_key: str = Field(..., min_length=1, max_length=32)
    category: str = Field(..., min_length=1, max_length=48)

    @field_validator("task_id", "task_title", "duration_key", "category", mode="before")
    @classmethod
    def normalize_micro_break_text(cls, value):
        return strip_non_empty_string(value, "micro break field")


class MicroBreakRewardInfo(BaseModel):
    xp: int
    coins: int


class MicroBreakSummaryResponse(BaseModel):
    micro_break_streak: int
    daily_completions: int
    daily_limit: int
    rewards: dict[str, MicroBreakRewardInfo]


class MicroBreakCompleteResponse(CompletionRewardResponse):
    micro_break_streak: int
    daily_completions: int
    daily_limit: int
