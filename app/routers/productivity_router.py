...
from app.models import Goal, GamificationEvent, Habit, HabitLog, HabitTypeEnum, KeyResultHistory, MicroTask, SubGoal, Task, TaskOccurrenceSkip, User
from app.schemas import (
    ...
    MicroBreakCompleteRequest,
    MicroBreakCompleteResponse,
    MicroBreakSummaryResponse,
    MicroBreakTaskCreate,
    MicroBreakTaskResponse,
    MicroBreakTaskUpdate,
    SubGoalCreate,
    SubGoalResponse,
    SubGoalUpdate,
    TaskCreate,
...
MICRO_BREAK_OPTIONS = {
    "5-10": {"category": "quick_reset"},
    "15-30": {"category": "productive_pause"},
    "30-60": {"category": "deep_recharge"},
    "relaxed": {"category": "relaxed_break"},
}
MICRO_BREAK_TASK_EVENT_TYPE = "micro_break_task"
MICRO_BREAK_TASK_SOURCE_TYPE = "micro_break_task"


def normalize_micro_break_duration(duration_key: str) -> str:
    if duration_key not in MICRO_BREAK_OPTIONS:
        raise HTTPException(status_code=400, detail="Durasi jeda tidak valid")
    return duration_key


def micro_break_category(duration_key: str) -> str:
    return MICRO_BREAK_OPTIONS[normalize_micro_break_duration(duration_key)]["category"]


def micro_break_task_response(task: MicroTask) -> MicroBreakTaskResponse:
    return MicroBreakTaskResponse(
        id=task.id,
        title=task.title,
        hint=task.hint,
...
def get_micro_break_task(db: Session, user: User, task_id: UUID) -> MicroTask | None:
    return db.query(MicroTask).filter(
        MicroTask.user_id == user.id,
        MicroTask.id == task_id,
        MicroTask.is_active.is_(True),
        MicroTask.deleted_at.is_(None),
    ).first()


def micro_break_task_query(db: Session, user: User, duration_key: str | None = None):
    query = db.query(MicroTask).filter(
        MicroTask.user_id == user.id,
        MicroTask.is_active.is_(True),
        MicroTask.deleted_at.is_(None),
    )
    if duration_key:
        duration_key = normalize_micro_break_duration(duration_key)
        query = query.filter(MicroTask.duration_key == duration_key)
    return query.order_by(MicroTask.created_at.desc())


def micro_break_daily_completions(db: Session, user: User, target_date: date) -> int:
    return db.query(GamificationEvent).filter(
        GamificationEvent.user_id == user.id,
        GamificationEvent.event_type == "completion_reward",
        GamificationEvent.source_type == "micro_break",
        GamificationEvent.event_date == target_date,
        GamificationEvent.deleted_at.is_(None),
    ).count()


def micro_break_streak(db: Session, user: User, today: date) -> int:
    rows = db.query(GamificationEvent.event_date).filter(
        GamificationEvent.user_id == user.id,
        GamificationEvent.event_type == "completion_reward",
        GamificationEvent.source_type == "micro_break",
        GamificationEvent.event_date.isnot(None),
        GamificationEvent.event_date <= today,
        GamificationEvent.deleted_at.is_(None),
    ).distinct().all()
    completed_dates = {row[0] for row in rows if row[0] is not None}
    streak = 0
    cursor = today
...
def micro_break_reward_config() -> dict[str, dict[str, int]]:
    return {key: {"xp": value["xp"], "coins": value["coins"]} for key, value in MICRO_BREAK_REWARDS.items()}


def micro_break_summary_payload(db: Session, user: User) -> MicroBreakSummaryResponse:
    today = local_date_for_user(user)
    return MicroBreakSummaryResponse(
        micro_break_streak=micro_break_streak(db, user, today),
        daily_completions=micro_break_daily_completions(db, user, today),
        daily_limit=MICRO_BREAK_DAILY_LIMIT,
        rewards=micro_break_reward_config(),
    )


@router.get("/micro-breaks/summary", response_model=MicroBreakSummaryResponse)
def get_micro_break_summary(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    return micro_break_summary_payload(db, current_user)


@router.get("/micro-breaks/tasks", response_model=List[MicroBreakTaskResponse])
def get_micro_break_tasks(
    duration_key: str | None = None,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    events = micro_break_task_query(db, current_user, duration_key).all()
    return [micro_break_task_response(event) for event in events]


@router.post("/micro-breaks/tasks", response_model=MicroBreakTaskResponse, status_code=status.HTTP_201_CREATED)
def create_micro_break_task(
    payload: MicroBreakTaskCreate,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    duration_key = normalize_micro_break_duration(payload.duration_key)
    now = utc_now()
    task = MicroTask(
        ...
        category=micro_break_category(duration_key),
        is_active=True,
        created_at=now,
        updated_at=now,
    )
    ...
    db.commit()
    db.refresh(task)
    return micro_break_task_response(task)


@router.put("/micro-breaks/tasks/{task_id}", response_model=MicroBreakTaskResponse)
def update_micro_break_task(
    task_id: UUID,
    payload: MicroBreakTaskUpdate,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    task = get_micro_break_task(db, current_user, task_id)
    if task is None:
        raise HTTPException(status_code=404, detail="Micro task tidak ditemukan")

    if payload.title is not None:
        task.title = payload.title.strip()
    if "hint" in payload.model_fields_set:
        task.hint = (payload.hint or "").strip() or None
    if payload.duration_key is not None:
        duration_key = normalize_micro_break_duration(payload.duration_key)
        task.duration_key = duration_key
        task.category = micro_break_category(duration_key)
...
@router.delete("/micro-breaks/tasks/{task_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_micro_break_task(
    task_id: UUID,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
...
@router.post("/micro-breaks/complete", response_model=MicroBreakCompleteResponse)
def complete_micro_break(
    payload: MicroBreakCompleteRequest,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    try:
        requested_task_id = UUID(payload.task_id)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail="Micro task tidak valid") from exc

    task = get_micro_break_task(db, current_user, requested_task_id)
    if task is None:
        raise HTTPException(status_code=404, detail="Micro task tidak ditemukan")

    duration_key = normalize_micro_break_duration(task.duration_key)
    category = task.category
    task_title = task.title
    reward = MICRO_BREAK_REWARDS[duration_key]

    today = local_date_for_user(current_user)
    daily_count = micro_break_daily_completions(db, current_user, today)
    if daily_count >= MICRO_BREAK_DAILY_LIMIT:
        raise HTTPException(status_code=409, detail="Batas reward micro break hari ini sudah tercapai")

    event = GamificationService.record_event(
        db,
        current_user,
        event_type="completion_reward",
        source_type="micro_break",
        source_id=requested_task_id,
        xp_delta=reward["xp"],
        coin_delta=reward["coins"],
        event_date=today,
        description=f"Micro break: {task_title} ({category})",
    )
    if event is None:
        summary = micro_break_summary_payload(db, current_user)
        return MicroBreakCompleteResponse(
            success=False,
            message="Tugas jeda ini sudah dihitung hari ini. Silakan ganti tugas lain.",
            xp_earned=0,
            coins_earned=0,
            penalty=0,
            new_level=current_user.level,
            new_xp=current_user.xp_balance,
            new_coins=current_user.coin_balance,
            xp_needed_for_next_level=GamificationService.xp_for_next_level(current_user.level),
            micro_break_streak=summary.micro_break_streak,
            daily_completions=summary.daily_completions,
            daily_limit=summary.daily_limit,
        )

    GamificationService.add_xp_and_check_level_up(db, current_user, reward["xp"])
    GamificationService.add_coins(db, current_user, reward["coins"], f"Micro break: {task_title}")
    db.commit()
    ...
    return MicroBreakCompleteResponse(
        success=True,
        message="Micro break selesai. Reward masuk ke sistem XP dan koin.",
        xp_earned=reward["xp"],
